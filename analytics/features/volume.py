...
class VolumeAnalytics:
    def analyze(self, features: pd.DataFrame) -> AnalysisResult:
        ...
        relative_volume = float(last.get("relative_volume", 0.0)) if pd.notna(last.get("relative_volume", 0.0)) else 0.0
        volume_spike = bool(last.get("volume_spike", False))
        dry_up = bool(last.get("volume_dry_up", False))
        accumulation = float(last.get("volume_accumulation", 0.0)) if pd.notna(last.get("volume_accumulation", 0.0)) else 0.0
