...
from cli.commands.doctor.strategies.auth_live_probe import AuthLiveProbeCheck
...
def test_auth_live_probe_probe_only_never_force_refreshes():
    ...
    with (
        ...
        patch(
            "cli.commands.doctor.strategies.auth_live_probe.list_available_brokers",
            return_value=[{"name": "dhan", "available": True}],
        ),
        patch(
            "cli.commands.doctor.strategies.auth_live_probe.authenticated_readiness_probe"
        ) as forced,
    ):
