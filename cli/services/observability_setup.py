...
def _collect_oms_risk_gauges(risk_manager: Any, service: BrokerService) -> dict[str, float]:
    ...
    try:
        gauges["capital_fallback_count"] = float(
            getattr(service, "_capital_fallback_count", 0)
        )
    ...
    if service._gateway is not None:
        try:
            ...
            gauges["market_stream_connected"] = 1.0 if conn_status.get("market_feed", False) else 0.0
            gauges["order_stream_connected"] = 1.0 if conn_status.get("order_stream", False) else 0.0
...
def start_http_observability(
    ...
    Best-effort: if bind fails (port in use), leaves server as None
