...
def show_cache_status(args: list[str], broker_service: BrokerService, console: Console) -> CommandResult:
    ...
    table.add_row("Status", "[green]Fresh (< 6h)[/green]" if age_hours < 6 else "[yellow]Stale (> 6h)[/yellow]")
...
def refresh_cache(args: list[str], broker_service: BrokerService, console: Console) -> CommandResult:
    ...
    try:
        ...
        console.print(f"[green]✅ Cache refreshed: {len(rows):,} instruments in {elapsed:.2f}s[/green]")
...
def show_cache_stats(args: list[str], broker_service: BrokerService, console: Console) -> CommandResult:
