...
def run_symbol_command(command: str, args: list[str], broker_service, console: Console) -> None:
    ...
    prices = gateway.history(
        symbol, timeframe="1D", lookback_days=120, from_date=str(from_date), to_date=str(to_date)
    )
    ...
    if command == "stock":
        ...
        chain = (
            gateway.option_chain(symbol) if hasattr(gateway, "option_chain") else {"strikes": []}
        )
        spot = (
