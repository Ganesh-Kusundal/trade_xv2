...
def run(args: list[str], broker_service, console: Console) -> None:
    ...
    try:
        ...
        # Find ATM
        atm_strike = min(strikes, key=lambda s: abs(float(s['strike']) - float(spot))) if strikes and spot else None
        ...
        # Find highest OI
        max_call_oi = max(strikes, key=lambda s: s.get('call', {}).get('oi', 0) or 0) if strikes else None
        max_put_oi = max(strikes, key=lambda s: s.get('put', {}).get('oi', 0) or 0) if strikes else None
        ...
        table.add_row("Highest Call OI", f"₹{max_call_oi['strike']:,.0f} (OI={max_call_oi.get('call', {}).get('oi', 0):,})" if max_call_oi else "N/A")
        table.add_row("Highest Put OI", f"₹{max_put_oi['strike']:,.0f} (OI={max_put_oi.get('put', {}).get('oi', 0):,})" if max_put_oi else "N/A")
