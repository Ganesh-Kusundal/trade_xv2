...
class FastBacktestEngine:
    """Optimized backtest engine that pre-computes features.

    Instead of running FeaturePipeline on a growing window for each bar,
    this engine runs the pipeline once on the full dataset, then iterates
    through the pre-computed features to generate signals.
    ...
    def _run_strategy(
        ...
        for idx in range(warmup, len(features)):
            ...
            if position is not None:
                unrealized = (float(row["close"]) - position["entry_price"]) * position["quantity"]
