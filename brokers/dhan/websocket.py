...
class DhanMarketFeed(ManagedService):
    ...
    def _publish_tick(self, quote: dict) -> None:
        ...
        try:
            ...
            self._event_bus.publish(
                DomainEvent.now(EventType.TICK, {"quote": q}, symbol=q.symbol, source="DhanMarketFeed")
            )
        except Exception as exc:
    ...
    def _publish_depth(self, depth: dict) -> None:
        ...
        try:
            ...
            self._event_bus.publish(
                DomainEvent.now(EventType.DEPTH, {"depth": md}, symbol=depth.get("symbol", ""), source="DhanMarketFeed")
            )
        except Exception as exc:
...
class DhanOrderStream(ManagedService):
    ...
    def _publish_order_update(self, data: dict) -> None:
        ...
        try:
            ...
            self._event_bus.publish(
                DomainEvent.now(EventType.ORDER_UPDATED, {"order": order}, symbol=order.symbol, source="DhanOrderStream")
            )
            ...
            # from the cumulative filled quantity so re-deliveries dedupe.
            #
            # Phase 2.1: the broker delivers ``filledQty`` as the cumulative
            # quantity for this order. We set ``Trade.cumulative_filled`` so
            # the OMS uses monotonic ``max(prev, cumulative)`` semantics and
            # out-of-order delivery cannot decrease the running fill count.
            # ``trade.quantity`` is set to ``filled`` for backward
            # compatibility with consumers that read the cumulative directly.
            ...
            if filled > 0 and avg > 0:
                ...
                self._event_bus.publish(
                    DomainEvent.now(EventType.TRADE, {"trade": trade}, symbol=trade.symbol, source="DhanOrderStream")
                )
        except Exception as exc:
