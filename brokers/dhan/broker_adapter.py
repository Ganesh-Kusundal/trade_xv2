...
"""Unified Dhan BrokerAdapter (data + execution + lifecycle).

Phase 9.2 of the Instrument-Centric SDK Redesign.

Combines Dhan's ``DataProvider`` and ``ExecutionProvider`` behind the single
...
class DhanBrokerAdapter:
    """Unified Dhan broker adapter: market data + order execution + lifecycle."""
    ...
    def authenticate(self) -> bool:
        """Dhan adapters are pre-authenticated at the gateway layer."""
        return True
