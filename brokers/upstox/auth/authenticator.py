"""Upstox authenticator — wraps :class:`UpstoxTokenManager`."""
...
class UpstoxAuthenticator:
    """Adapter around :class:`UpstoxTokenManager` for Upstox (OAuth / TOTP)."""
    ...
    def acquire(self) -> str:
        try:
            state = self._mgr.bootstrap()
        except Exception as exc:
            raise BrokerAuthError(f"Upstox auth failed: {exc}") from exc
        token = state.access_token
        if not token:
            raise BrokerAuthError("Upstox bootstrap returned no access token")
        self._fire_refresh(token)
        return token

    def is_authenticated(self) -> bool:
        try:
            return bool(self._mgr.current_token())
        except Exception:
            return False

    def ensure_valid(self) -> bool:
        try:
            self._mgr.ensure_valid()
            return bool(self._mgr.current_token())
        except Exception:
            return False
