...
bal = gw.get_balance()
check("get_balance() returns Balance", isinstance(bal, Balance), f"available={bal.available_balance}")
funds = gw.funds()
...
check("depth() returns MarketDepth", isinstance(d, MarketDepth), f"bids={len(d.bids)}, asks={len(d.asks)}")
...
for tf in aliases:
    try:
        df = gw.history("RELIANCE", "NSE", timeframe=tf, lookback_days=5)
        check(f"history alias '{tf}'", isinstance(df, pd.DataFrame) and len(df) > 0, f"{len(df)} candles")
    except Exception as e:
...
try:
    ...
    if atm_strike:
        ...
        check("CALL greeks populated", has_call_greeks, f"delta={call.greeks.get('delta') if call.greeks else 'None'}")
        check("PUT greeks populated", has_put_greeks, f"delta={put.greeks.get('delta') if put.greeks else 'None'}")
    time.sleep(1.1)
except Exception as e:
...
try:
    ...
    check("option_chain(BANKNIFTY)", isinstance(chain2, OptionChain) and len(chain2.strikes) > 0, f"{len(chain2.strikes)} strikes")
    time.sleep(1.1)
except Exception as e:
...
try:
    ...
    check("future_chain has contracts", len(fc.contracts) > 0, f"{len(fc.contracts)} contracts, expiries={len(fc.expiries)}")
except Exception as e:
...
check("ltp_batch() → dict[str, Decimal]", isinstance(ltp_b, dict) and all(isinstance(v, Decimal) for v in ltp_b.values()), f"keys={list(ltp_b.keys())}")

# quote_batch
q_b = gw.quote_batch(["RELIANCE", "TCS"], "NSE")
check("quote_batch() → dict[str, Quote]", isinstance(q_b, dict) and all(isinstance(v, Quote) for v in q_b.values()), f"keys={list(q_b.keys())}")

# history_batch
try:
    h_b = gw.history_batch(["RELIANCE", "TCS"], "NSE", timeframe="1D", lookback_days=5)
    check("history_batch() → DataFrame", isinstance(h_b, pd.DataFrame) and len(h_b) > 0, f"{len(h_b)} rows")
except Exception as e:
...
check("positions() → list[Position]", isinstance(positions, list) and all(isinstance(p, Position) for p in positions), f"{len(positions)} positions")

holdings = gw.holdings()
check("holdings() → list[Holding]", isinstance(holdings, list) and all(isinstance(h, Holding) for h in holdings), f"{len(holdings)} holdings")

trades = gw.trades()
check("trades() → list[Trade]", isinstance(trades, list) and all(isinstance(t, Trade) for t in trades), f"{len(trades)} trades")

# Verify Trade.timestamp is datetime, not string
if trades:
    from datetime import datetime
    t0 = trades[0]
    check("Trade.timestamp is datetime|None", t0.timestamp is None or isinstance(t0.timestamp, datetime), f"type={type(t0.timestamp).__name__}")
...
ob = gw.get_orderbook()
check("get_orderbook() → list[Order]", isinstance(ob, list) and all(isinstance(o, Order) for o in ob), f"{len(ob)} orders")

tb = gw.get_trade_book()
check("get_trade_book() → list[Trade]", isinstance(tb, list) and all(isinstance(t, Trade) for t in tb), f"{len(tb)} trades")
...
check("search() → list[dict]", isinstance(search_results, list) and len(search_results) > 0, f"{len(search_results)} results")
