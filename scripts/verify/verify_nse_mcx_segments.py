...
def test_nse_equity(gw):
    ...
    try:
        ...
        record(
            "Quote",
            "NSE",
            "PASS" if quote and quote.ltp else "FAIL",
    ...
    try:
        ...
        record(
            "Depth (5-level)", "NSE", "PASS" if bids > 0 else "FAIL", f"{bids} bids, {asks} asks"
        )
    except Exception as e:
    ...
    try:
        ...
        record(
            "History (1D)",
            "NSE",
            "PASS" if df is not None and len(df) > 0 else "FAIL",
...
def test_nse_fno(gw):
    ...
    try:
        ...
        record("Option Chain", "NFO", "PASS" if strikes > 0 else "FAIL", f"{strikes} strikes")
    except Exception as e:
...
def test_mcx_commodity(gw):
    ...
    try:
        ...
        record("Quote", "MCX", "PASS" if quote and quote.ltp else "FAIL", f"LTP={quote.ltp}")
    except Exception as e:
    ...
    try:
        ...
        asks = len(depth.asks) if depth and hasattr(depth, "asks") else 0
        record(
            "Depth (5-level)", "MCX", "PASS" if bids > 0 else "FAIL", f"{bids} bids, {asks} asks"
        )
    except Exception as e:
    ...
    try:
        ...
        strikes = len(chain.strikes) if chain and hasattr(chain, "strikes") else 0
        record("Option Chain", "MCX", "PASS" if strikes > 0 else "FAIL", f"{strikes} strikes")
    except Exception as e:
...
def test_batch_operations(gw):
    ...
    try:
        ...
        record(
            "LTP Batch",
            "NSE",
            "PASS" if batch and len(batch) > 0 else "FAIL",
            f"{len(batch)} symbols",
        )
    except Exception as e:
