...
def main():
    ...
    try:
        ...
        try:
            ...
            expiry = -1
            print(
                f"  ℹ Info: User Profile fetch failed (expected if current IP is not the registered Static IP): {exc}"
            )
        if expiry > 0:
            ...
            print(
                "  ℹ Info: User Profile fetch skipped/failed (expected if current IP is not the registered Static IP)."
            )
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(
                    f"  ✓ Market Quote (LTP) successful! RELIANCE Last Traded Price: {reliance_data.get('last_price')}"
                )
                quote_success = True
            else:
                print(
                    f"  ✗ Market Quote request failed with status {resp.status_code}: {resp.text}"
                )
                quote_success = False
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if candles:
                    print(
                        f"  ✓ Historical Candles successful! Retrieved {len(candles)} days of candles. Latest close: {candles[0][4]}"
                    )
                else:
                    print(
                        "  ✓ Historical Candles request successful, but empty candle list returned."
                    )
                historical_success = True
            else:
                print(
                    f"  ✗ Historical Candles request failed with status {resp.status_code}: {resp.text}"
                )
