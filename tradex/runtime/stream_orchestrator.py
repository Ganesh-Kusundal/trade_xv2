...
# ---------------------------------------------------------------------------
# Exchange-time preservation + dedup
# ---------------------------------------------------------------------------

# Market ticks are de-duplicated across reconnect/backfill overlaps using a
...
def _parse_exchange_time(ts_raw: Any, now: datetime) -> datetime:
    ...
    Upstox carries ``exchange_timestamp`` (epoch milliseconds). Dhan quotes have
    no timestamp at all. When the incoming value is missing, non-positive, or
    unparseable we preserve the local arrival time so downstream ordering and
    freshness stay well-defined.
    """
    if ts_raw is None:
        return now
    try:
        if isinstance(ts_raw, (int, float)):
            if ts_raw <= 0:
                return now
            # Upstox ltt is epoch milliseconds; guard against second-resolution too.
            if ts_raw > 1e11:
...
class StreamOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        # Optional live tick→candle aggregator. OFF by default so existing
        # behavior is unchanged. Feed ticks via attach_candle_aggregator() or
        # this constructor arg. Aggregation is cheap and non-blocking (bounded
        # per-symbol state, synchronous, no I/O).
        self._candle_aggregator: "CandleAggregator | None" = candle_aggregator
    ...
    def attach_candle_aggregator(self, aggregator: "CandleAggregator") -> None:
        ...
        (synchronously and cheaply) in addition to registered consumers. Safe to
        call at runtime; pass ``None`` to detach.
        """
    ...
    async def _deliver_tick(self, session_id: str, tick: MarketTick) -> None:
        ...
        # Optional live candle aggregation. Synchronous, bounded, and I/O-free;
        # an aggregator failure must never break tick fan-out, so it is guarded.
    ...
    def _dedup_drop(
        ...
        Keyed on ``(instrument, exchange_time, sequence)``. After a reconnect or
    ...
    def _normalize_tick(
        ...
        if not symbol:
            return None
        # Preserve the exchange timestamp when the quote carries one
        # (Upstox); fall back to local arrival time (Dhan / absent).
        event_time = _parse_exchange_time(
            frame.get("timestamp") or frame.get("exchange_timestamp"), now
        )
        return MarketTick(
