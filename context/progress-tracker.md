...
- **v2/ broker-layer: remaining gap-analysis items + live Dhan verification (2026-07-23):** Closed the 3 items explicitly deferred from the broker gap-analysis pass above (previously flagged as separate feature-sized work; user asked to fix them too). (1) **Instrument resolution**: `plugins/brokers/common/instruments.py` — `ResolvedInstrument`/`BrokerWireRef` canonical-vs-wire split + `InMemoryInstrumentResolver` (register/load_from_rows/resolve_ref/reverse); `DhanWire`/`UpstoxWire` now delegate to an instance of it instead of their own private dicts (same public methods, no call-site changes, behavior-preserving). No live instrument-master fetch integration — that data-loading piece is still separate, unverified work; the resolver is ready for it. (2) **Token-lifecycle depth**: `plugins/brokers/common/token_lifecycle.py` — `TokenLifecyclePort` protocol (formalizes the `ensure_token`/`current` shape both token managers already had), `TokenBroadcast` (registered receivers get the new token when one is minted), `TokenRefreshScheduler` (background thread, non-forcing — only checks, `ensure_token()` itself is still probe-before-mint). Deliberately **not** a copy of legacy's weak-ref `ConnectionTokenManager`: writing the test caught that weak refs silently drop a receiver with no other strong reference (an inline lambda), which legacy's many-short-lived-subscribers use case justified but v2's one-receiver-per-connection-lifetime case doesn't — switched to a plain strong-ref list instead. Also deliberately **not** importing `infrastructure.component.base.Component` for the scheduler: that would make `plugins/` depend on `infrastructure/`, a boundary nothing in `plugins/` currently crosses and the spec's own dependency rule (2.2) says it shouldn't — scheduler owns its own minimal start/stop instead. Wired `register_receiver()` into both `DhanTokenManager`/`UpstoxTokenManager`'s actual mint paths (TOTP generate / refresh-token exchange), broadcasting on real mint, not just as a standalone module. (3) **Extension mechanism**: `plugins/brokers/common/extensions.py::BrokerExtensions` — register/get-by-type/names, `LookupError` with available-names on miss. Wired a `.extensions` registry + `.extension(type)` lookup onto all three gateways (empty by default — this is the seam, not a shipped extension; no fabricated Dhan super-order/EDIS feature added since nothing pins down its real behavior to build against safely). **Live verification**: ran `PYTHONPATH=src python -m tradex.check_connection --broker dhan` twice (before and after all changes above) against the real LIVE credentials in `v2/.env.local` — user explicitly approved this since it touches a real financial account (read-only: authenticate + get_funds + get_positions, no orders) — both runs: `authenticate=True, funds balance=0.34 INR, positions=0, PASS`. Confirms `DhanGateway → DhanConnection → DhanTokenManager → HttpTransport → real Dhan API → DhanWire → domain objects` works end-to-end and was unaffected by the wire/token/extension refactors. Verified (offline): 894 passed, 2 skipped (was 867 before this pass, +27 new tests), import-linter 5/5 contracts KEPT.
- **v2/ broker-layer gap analysis + fixes vs legacy src/brokers (2026-07-23):** Compared `v2/src/plugins/brokers` against the mature `src/brokers` (307 files: gateway.py, session/broker_session.py, common/auth/lifecycle.py, providers/dhan/auth/{token_manager,connection_token_manager,token_scheduler,totp_client,edis,ip_management}.py, common/instruments/{carrier,service}.py, providers/dhan/{resolver,resolver_refresher,symbol_validator}.py, common/wire_base.py, infrastructure/resilience/transport_errors.py, common/contracts/broker_contract.py, extensions/broker_extension.py). Found v2's broker layer materially thinner: no shared token-lifecycle protocol/background refresh scheduler/live-token-broadcast, `SymbolResolver` deleted as dead code in REF-9 without a replacement (legacy proves the pattern is load-bearing), no canonical broker-error taxonomy, no extension point for broker-unique capabilities (Dhan super orders/EDIS/etc.), no unified connection-liveness contract. Full write-up not saved to a file — see this session's chat transcript for the complete bullet-by-bullet gap analysis and recommendations. Fixed the 3 items scoped as small/contained (deferred: SymbolResolver rebuild, token-lifecycle depth, extension mechanism — each is separately-scoped feature work): (1) added `BrokerError`/`AuthenticationError`/`RateLimitError`/`NetworkError` to `shared/errors.py`, wired into `plugins/brokers/common/transport.py` (401/403→auth, 5xx→network, other 4xx→broker, 429 stays `RateLimitExceeded` now also a `RateLimitError`) and `retry.py`/`circuit_breaker.py`'s existing exceptions (multiple-inheritance, backward compatible); (2) added `plugins/brokers/common/liveness.py::ConnectionLiveness` mixin (pins `is_connected` = connected AND authenticated via a `_transport_connected()` hook) to prevent the exact is_connected semantic drift legacy hit once between Dhan/Upstox — wired into all three connection classes; (3) strengthened `tests/integration/adapter_harness.py`'s `AdapterTestHarness` (no-raw-wire-dict-leak assertions on positions/funds/mass_status, capability-declaration non-empty check) — which surfaced a real bug caught by the new assertion: `DhanGateway.mass_status()`/`UpstoxGateway.mass_status()` returned a raw dict instead of `BrokerSnapshot` (Paper's was already correct); fixed both, and added `supported_asset_classes` to all three brokers' capability declarations (previously undeclared/empty). Verified: 867 passed, 2 skipped (was 859 pre-session, +8 new tests via this pass), import-linter 5/5 contracts KEPT.
- **v2/ spec-gap remediation (2026-07-23):** Reviewed `v2/` against `specs/` (00-15 + IMPLEMENTATION-SPEC.md) end-to-end; findings and full plan at `/Users/apple/.claude/plans/fluffy-gliding-newt.md`. Fixed the small, contained subset (P0-P3), left the feature-sized gaps (Indian risk rules, BrokerHealthMonitor, Parquet/DuckDB, indicator library, CLI breadth, OTLP, CI stages) explicitly deferred. **P0 (live bug):** `ExecutionEngine._now()` (`application/execution/execution_engine.py`) passed the real `Clock.now()` return value (`domain.value_objects.Timestamp`, nanosecond int) straight into `OrderPlaced`/`OrderFilled`/`RiskBreached` `timestamp=` fields typed `datetime` — wrong type on every booted node (PAPER/LIVE/REPLAY/BACKTEST, confirmed via `runtime/factory.py` wiring, not test-only). Fixed with `datetime.fromtimestamp(ts.value / 1e9, tz=UTC)`; regression test in `test_execution.py`. **P1 (dedup):** deleted dead `infrastructure/message_bus/message_log.py` (unused duplicate of `log.py`) and the empty stray `infrastructure/message_log/` dir; unified `ComponentState` (`infrastructure/component/base.py` now imports the `domain.enums` StrEnum instead of redefining its own `Enum`); re-typed `observability/health.py`'s `ComponentHealth.state` to that enum instead of a bare `str`. Kept `domain.events.RiskCheckResult` — initially flagged as a dead duplicate of `application.risk.context.RiskCheckResult`, but reverted after `tests/unit/domain/test_messages.py` turned out to deliberately test it as part of the spec's message-hierarchy catalog (it's a distinct bus-message concept, just never published yet). **P2 (wiring):** added optional `audit_sink` param to `ExecutionEngine`, wired in `runtime/factory.py`, records at all 4 order-path points (received/risk-check/placed/fill). Added `ExecutionEngine.trip_kill_switch(reason)` (cancels open orders, publishes `Shutdown`) and `ExecutionEngine.reconcile(broker_orders=, broker_positions=)` (calls the previously-orphaned `application/reconciliation/engine.py::ReconciliationEngine`, publishes `ReconciliationDrift`/`ReconciliationCompleted`) — no auto-trigger-on-reconnect yet, that needs a connection-lifecycle hook that doesn't exist. **P3 (contracts):** added the missing 5th import-linter contract (dhan/upstox/paper mutual independence) to `pyproject.toml`. Running it surfaced a real pre-existing violation outside the original findings: `domain/ports/risk_model.py` imported `application.risk.context.RiskCheckResult` under `TYPE_CHECKING` (import-linter counts it regardless of the guard) — fixed by typing the Protocol's return as `Any` instead. Verified: 859 passed, 2 skipped (was 854, +5 new tests, 0 regressions); `lint-imports`: 5/5 contracts KEPT.
- **Full v2/ Refactoring Complete (2026-07-23):** Completed all 9 refactoring tasks (REF-1, REF-2, REF-3, REF-5, REF-7, REF-8, REF-9, REF-10, REF-11) from the deep architectural audit. 838 tests pass (0 failures, 2 skipped). Fixed domain purity test to handle TYPE_CHECKING imports. All findings from the audit addressed: dual message hierarchy eliminated, RiskCheckResult consolidated, Environment/BrokerId consolidated, ExecutionEngine OMS boundary violation fixed, RiskContext made meaningful, StrategyEngine uses features, dead code deleted, inline imports fixed, Runtime frozen dataclass contradiction resolved.

- **REF-9 (2026-07-22):** Deleted 5 dead-code files and implemented real CLI commands. Deleted: `v2/src/plugins/brokers/common/wire_mapper.py` (WireMapper — not imported by any adapter; DhanWire/UpstoxWire implement their own mapping), `v2/src/plugins/brokers/common/symbol_resolver.py` (SymbolResolver — not imported by any adapter; DhanWire/UpstoxWire use module-level dicts), `v2/src/infrastructure/message_bus/router.py` (MessageRouter/RouteBuilder — not imported by any application code), `v2/src/application/strategy/engine.py` (parallel StrategyEngine class — `strategy_engine.py` is the live version imported by `application/analytics/engines.py`), `v2/tests/unit/plugins/test_symbol_resolver.py` (tests for deleted symbol_resolver.py). Updated `__init__.py` re-exports (removed WireMapper/SymbolResolver from `plugins/brokers/common/__init__.py`, removed MessageRouter from `infrastructure/message_bus/__init__.py`, switched `application/strategy/__init__.py` to import from `strategy_engine.py`). Added `start_strategy`/`stop_strategy` methods to `strategy_engine.py` (previously only in deleted `engine.py`). Cleaned up test files: removed `TestWireMapper`/`TestSymbolResolver` from `test_broker_common.py`, removed `TestMessageRouter` from `test_message_bus.py`, updated `test_strategy.py` import. Implemented real CLI commands in `interface/cli.py` (replaced "(stub)" prints with argument-accepting commands for replay/backtest/paper). Tests: 835 passed, 2 skipped, 3 pre-existing failures (domain purity violation in `risk_model.py` from REF-1's `domain/messages.py` deletion — unrelated to REF-9).

- **REF-7 (2026-07-22):** Fixed `ExecutionEngine._risk_context()` to build a meaningful `RiskContext` from the actual `TradingCache` state instead of hardcoded empty values. Changes in `v2/src/application/execution/execution_engine.py`: (1) Added `self._order_count = 0` counter in `__init__`, incremented after each successful fill in `submit()`. (2) Rewrote `_risk_context()` to pull real positions from `self._order_manager._cache` (or `self._store._cache` if `_TradingCacheStore`), compute `daily_pnl` as the sum of positions' `realized_pnl`, and use the tracked `order_count`. (3) Added `_get_positions()`, `_compute_daily_pnl()`, and `_compute_available_margin()` helper methods. (4) Added `InstrumentId` and `Position` imports. Previously positions was always `{}`, `order_count` was always `0`, and `daily_pnl` was always `0` — making `PositionLimitRule` and `OrderRateRule` no-ops. Tests: 19 passed, 0 failed (`test_risk_manager.py`, `test_risk_rules.py`, `test_execution_engine.py`, `test_four_mode_parity.py`). Consolidated `Environment` and `BrokerId` into `domain/enums.py` as the single source of truth. `config/schema.py` now imports both from `domain.enums` instead of redefining them (removed `str, Enum` Environment class and `Literal["dhan","upstox","paper"]` BrokerId). Updated `config/schema.py` default `broker: BrokerId = BrokerId.PAPER` (was `"paper"`). Updated `runtime/broker_factory.py` to compare against `BrokerId.PAPER`/`BrokerId.DHAN`/`BrokerId.UPSTOX` instead of lowercase strings. Updated all 5 config YAML files (`tradex.yaml`, `profiles/{paper,live,replay,backtest}.yaml`) from lowercase to uppercase broker values. Updated `tests/unit/config/test_loader.py` (YAML content, `TRADEX_BROKER` env var, assertion) and `tests/component/test_live_broker_boot.py` (`broker=BrokerId.DHAN`). Tests: 37 specified + 2 live boot + 109 domain/plugin/discovery + 23 execution/startup = 171 passed, 0 failed.

- **REF-2 (2026-07-22):** Consolidated `RiskCheckResult` into single definition at `application/risk/context.py`. Removed try/except ImportError fallback in `application/execution/protocols.py` (direct import now). Updated `domain/ports/risk_model.py` to import from `application.risk.context` via `TYPE_CHECKING` guard (avoids circular import: `application.risk.context` → `domain.entities` → `domain.__init__` → `domain.ports` → `domain.ports.risk_model` → `application.risk.context`). Updated `tests/unit/domain/test_ports.py` import to match. Note: `domain/events/__init__.py` still defines a separate `RiskCheckResult` domain event (subclass of `RiskMessage`) — this is a distinct concept (domain event with `timestamp`/`correlation_id`/`source` and `Quantity`/`Price` types) from the application DTO (`Decimal | None` optional fields). Tests: 82 passed, 0 failed.

- **REF-10 (2026-07-22):** Fixed inline imports across 3 files. `application/execution/execution_engine.py`: moved `Decimal` and `RiskContext` to top-level; removed try/except ImportError fallback in `_risk_context()` (direct import per REF-2 consolidation). `tradex/check_connection.py`: moved `DhanGateway`, `DhanConfig`, `UpstoxGateway`, `UpstoxConfig`, `TotpRateLimitError`, `load_v2_env` to top-level; removed all inline imports from `main()` and `_check()`. `plugins/brokers/paper/connection.py`: moved `datetime`, `Quote`, `Price`, `Quantity` to top-level; removed inline imports from `set_quote()`. Tests: 53 passed, 1 pre-existing failure (`test_paper_session_configure_start_stop` — pydantic enum validation, unrelated to import refactoring).

- **REF-1 (2026-07-22):** Merged `domain/messages.py` (332 lines, int nanosecond timestamps) into `domain/events/__init__.py` (75 lines, datetime timestamps) to eliminate the dual message hierarchy. All 35 unique classes from `messages.py` merged into `events/__init__.py` with `datetime` timestamps per spec. Category markers (`MarketDataMessage`, `OrderMessage`, `PortfolioMessage`, `RiskMessage`, `SystemMessage`, `RankMessage`) now inherit from `DomainEvent`. Overlapping classes (`OrderPlaced`, `OrderFilled`, `OrderCancelled`, `OrderRejected`) inherit from `OrderMessage` (which inherits from `DomainEvent`). `ReplayStarted.start_ts/end_ts` and `FeatureComputed.feature_timestamp` converted from `int` to `datetime`. `ReconciliationCompleted.duration_ms` kept as `int` (duration, not timestamp). Deleted `domain/messages.py`. Updated 6 import sites: `wire_mapper.py`, `risk_model.py`, `test_broker_common.py`, `test_message_bus.py`, `test_ports.py`, `test_messages.py`. Updated `MessageLog` protocol + `InMemoryMessageLog.read` + `MessageBus.replay` to use `datetime | None` for start/end. Updated `test_message_bus.py` and `test_broker_common.py` to use `datetime` timestamps. Tests: 854 passed, 2 skipped.

- **V2 Broker Module — Deep Architectural Audit (2026-07-23):** Completed full PHASE 1–5 audit of `v2/src/plugins/brokers/`. Produced 60 findings (SMELL-1 through SMELL-60) across 7 root cause categories. 16-task refactoring plan (REF-1 through REF-16) with dependency-ordered sequencing. Audit written to `v2/ARCHITECTURAL-AUDIT.md`. Key findings: 7 HIGH (duplicated gateway/connection/adapter structures, module-level mutable dicts, frozen dataclass mutation bug), 12 MEDIUM, 41 LOW. Dead code: `wire_mapper.py`, `symbol_resolver.py` (unused abstractions).

- **V2 Broker Module — Hard-Coded Values Audit & Fixes (2026-07-22):** Completed full PHASE 1–5 audit of `v2/src/plugins/brokers/`. Fixed 2 failing tests (module-level dict shared state → instance-level dicts in DhanWire/UpstoxWire). All 854 tests pass (0 failures, 2 skipped). Token reuse behavior verified (18 Dhan + 6 Upstox tests). Produced comprehensive shotgun surgery / structural inconsistency / coupling defect analysis with prioritized refactoring plan.
...
- **Phase 4 WS-H (2026-07-22):** Paper/replay hook parity decision — ADR-0024 documents four `SignalProcessorHooks` differences (`entry_gate`, `equity_for_sizing`, `position_view`, `slippage_pct`) as INTENTIONAL simulation-fidelity choices (replay `entry_gate=False` preserves unconstrained historical signal coverage); integration regression `test_paper_replay_hook_parity.py`.

- **Phase 4 WS-G (2026-07-22):** ProcessKernel.boot() unification — `ProcessKernel.wire()` / `ProcessKernel.boot(mode)` in `runtime/kernel.py` as sole entry calling `wire_domain_port_sinks()` + `wire_runtime_hooks()` once; modes delegate to `build_for_api`, `build_runtime`, `bootstrap_platform`, `build(trade)`; API `create_app` and `tradex.session` route through kernel; `build_connect_oms` replaces inline `build_oms_service` in session; compose import wires via kernel. Ratchet: `test_single_composition_root.py`.

- **Phase 4 WS-B (2026-07-22):** Broker timestamp preservation — shared `brokers/common/quote_timestamp.parse_quote_exchange_time()`; `normalize_broker_quote()` sets `DataProvenance.provider_timestamp` while `fetched_at` stays ingest wall-clock; Dhan `SubscriptionEngine` passes quote timestamps; Upstox depth stream uses broker time + `DepthKind` enum. Tests: `test_quote_normalize_provider_timestamp.py`, `test_subscription_engine_quote_timestamp.py`, `test_streaming_gateway_depth_timestamp.py`.

- **Phase 4 WS-C (2026-07-22):** Datalake timestamp bugs — `DataLakeGateway.query_candles()` converts naive-UTC API epoch filters to naive IST before Parquet compare; `HistoricalDataService.cache_to_parquet()` routes through `ensure_timestamp_dtype()`; `/market/candles` `X-Data-Freshness` now explicit IST (+05:30) aligned with lake `X-Data-Last-Update`. Tests: `test_gateway_timestamp_filter.py`, `test_historical_data_cache_normalizes_timestamp.py`.

- **Phase 4 WS-D (2026-07-22):** Option/future chain ingress provenance — `OptionChain` / `FutureChain` gain trailing `fetched_at: datetime | None`; `option_chain_from_dict` / `future_chain_from_dict` stamp via `get_current_clock().now()` when absent; `to_dict` round-trips ISO timestamp. Tests: `test_options_chain_provenance.py`.

- **Phase 4 R23 (2026-07-22):** Multi-asset historical redesign (ADR-0023) — contract-centric identity, lane routing, `ContractHistoricalCoordinator`, contract lake + MCX plugin, API `/historical/{equities|options|futures}`, CLI sync/parity. **Principal-engineer review fixes:** restored Dhan/Upstox `orders` rate limits + `extra_windows` in `rate_limit_config.py` (real-money safety); MCX futures route via `_asset_kind` FUT-before-kind (COMMODITY display kind no longer breaks routing); contract manifest no longer bootstraps fake rolling `InstrumentId`s from options catalog — rolling groups stay on `options_sync_manifest.csv` only; `ContractViews` wired in `ViewManager`; legacy `all_candles` aliased to `v_candles_1m`. Index spot OHLCV remains `sync_manifest.csv` → `v_candles_1m` (246M rows).

- **Phase 4 R22 (2026-07-22):** Architecture contract audit P1 — `order_idempotency.py` requires `correlation_id` or `X-Idempotency-Key` on POST `/orders` (dev auto-key only with `TRADEX_DEV=1`); `_trades.py` declares explicit `require_auth`; extended live order mutations gated by `require_extended_order_spine_allowed` (blocked under ADR-0012 paper default + production/staging until unified OMS spine). Tests: `test_order_idempotency.py`, `test_api_contract_p1.py`, `test_auth.py`.

- **Phase 4 R21 (2026-07-22):** Architecture contract audit P0 remediation — bootstrap passes `trading_context.order_manager` into `create_composers_from_infra` (fixes split-brain OMS R1); `enforce_live_order_authority` risk_payload uses mapped domain enum values (SL/DELIVERY no longer fail risk modelling); integration tests `test_api_order_oms_parity.py` (POST→GET→DELETE same book); bootstrap wiring tests use lightweight factory + block `.env.local` live bootstrap (fixes hang). Tests: `test_api_oms_singleton.py`, `test_api_order_oms_parity.py`, `test_api_bootstrap_wiring.py`.

- **Phase 4 R20 (2026-07-22):** ExecutionComposer sync bridge P0-2 — `_place_via_oms`/`_cancel_via_oms`/`_modify_via_oms` use `run_coro_sync` (not `asyncio.run`); cancel default fail-closed (`getattr(resp, "success", False)`); `execution_target_kind` injected at construction (factory resolves via `runtime.execution_config.resolve_execution_target_kind()`). Tests: `test_execution_composer_sync_bridge.py`.

- **Phase 4 R19 (2026-07-22):** API order contract audit P0-3/P0-4 — `interface/api/order_mapping.py` maps API wire enums (`SL`→`STOP_LOSS`, `SL-M`→`STOP_LOSS_MARKET`, `DELIVERY`→`CNC`) before domain enum construction in `place_order`/`modify_order`; cancel/modify routes call `ExecutionComposer` directly (no `om.cancel_order`/`om.modify_order` double-nesting). Tests: `test_order_mapping.py`, `test_api_order_routes.py`.

- **Phase 4 R18 (2026-07-22):** Federated options historical sync — replaced Trade_J DuckDB ETL with Dhan rolling API (`OptionsHistoricalFetchPort`, `OptionsHistoricalCoordinator`, `DhanRollingOptionsFetcher`, `build_federated_options_fetch_fn`); manifest `data/lake/options_sync_manifest.csv` (6 groups: NIFTY/BANKNIFTY × WEEK 1/2 + MONTH 1); `sync_options.py` accepts injected fetch port; `runtime/options_sync.py` + `tradex datalake sync-options` + `scripts/sync_options_datalake.py` + `--with-options` on equity sync; readers fixed (`DataLakeGateway.option_chain`, `options_greeks.py` → `options/candles/`); `convert_from_dhan_rolling` in `option_format.py`; `OperationKind.GET_OPTIONS_HISTORICAL` + `options_historical` quota class. Tests: `test_option_format.py`, `test_gateway_option_chain.py`, `test_options_sync_manifest.py`, `test_no_trade_j_in_datalake.py`. Run off-hours: `tradex datalake sync-options` (serial fetch, ~20–30 min full catch-up).
