    ...
    *,
    ...
    if df is None or df.empty:
        return HistoricalSeries(
            bars=[], coverage=DateRange(date.today(), date.today()),
    ...
    for idx, row in df.iterrows():
        ...
        bars.append(HistoricalBar(
            instrument=instrument, timeframe=timeframe, event_time=event_time,
            ...
            provenance=DataProvenance.now(
                ...
                provider_timestamp=event_time, transformation_chain=transformation_chain,
            ),
            label_convention=BarLabelConvention.LEFT,
        ))
    bars.sort(key=lambda b: b.event_time)
    return HistoricalSeries(
        bars=bars, coverage=coverage_from_bars(bars),
        instrument=instrument, timeframe=timeframe,
    )
    ...
    *,
    ...
    for idx, row in df.iterrows():
        ...
        bars.append(HistoricalBar(
            ...
            bar_index=int(idx) if isinstance(idx, int) else len(bars),
            provenance=DataProvenance.now(
                broker_id="datalake", request_id=request_id,
...
def from_dataframe(
    cls, df, instrument: InstrumentRef, timeframe: str,
) -> HistoricalSeries:
    """Replay/backtest ingress — delegates to from_broker_df."""
    return from_broker_df(
        cls, df, instrument, timeframe,
        broker_id="replay", request_id="from_dataframe",
