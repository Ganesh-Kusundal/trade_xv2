...
class InstrumentState:
    ...
    def with_unsubscribed(self) -> InstrumentState:
        """Return a new state with the subscription torn down."""
        return InstrumentState(
            quote=self.quote,
            depth=self.depth,
            subscription=self.subscription.with_status(SubscriptionStatus.UNSUBSCRIBED),
            last_update=self.last_update,
            error=None,
        )
