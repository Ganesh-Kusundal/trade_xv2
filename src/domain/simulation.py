"""Domain-level simulation constructs — portfolio projection, fill pipeline, position metadata.

These are pure domain constructs (depend only on ``domain.entities``, ``domain.fill_reducer``,
``domain.symbols``). They were previously in ``application.services.simulation_orchestrator``
with backward-compat shims in ``domain/``. This file is the canonical domain home.

Live OMS (:class:`application.oms.position_manager.PositionManager`), replay, and
paper must apply fills through :class:`PortfolioProjector` before mode-local read
models diverge.
"""
