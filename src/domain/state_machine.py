...
Domain logic (relocated from ``infrastructure.state_machine`` during the D4
OMS→infra port extraction). State transitions are a domain concern; the
infrastructure module now re-exports these symbols for backward compatibility.
...
class StateMachine(Generic[T]):
    ...
    def reset(self, new_state: T | None = None) -> None:
        """Reset the state machine to a new state (bypasses validation)."""
