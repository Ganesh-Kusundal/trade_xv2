...
from interface.api.deps import get_container, initialize_all_services
...
def create_app(
    ...
    **additional_services:
    ...
    from interface.api.auth import configure as _configure_auth
    ...
    if trading_context is None and event_bus is not None:
        from interface.api.lifecycle import build_trading_context
    ...
    from interface.api.middleware import RateLimitMiddleware, RequestLoggingMiddleware
    ...
    # Symbol endpoints
    from interface.api.routers.symbols import router as symbols_router
    ...
    # Analytics endpoints
    from interface.api.routers.analytics import router as analytics_router

    app.include_router(analytics_router, prefix=f"{cfg.api_prefix}/analytics", tags=["Analytics"])

    # Scanner endpoints
    from interface.api.routers.scanner import router as scanner_router

    app.include_router(scanner_router, prefix=f"{cfg.api_prefix}/scanner", tags=["Scanner"])

    # Strategy endpoints
    from interface.api.routers.strategy import router as strategy_router
    ...
    # Backtest endpoints
    from interface.api.routers.backtest import router as backtest_router

    app.include_router(backtest_router, prefix=f"{cfg.api_prefix}/backtest", tags=["Backtest"])

    # Portfolio endpoints
    from interface.api.routers.portfolio import router as portfolio_router

    app.include_router(portfolio_router, prefix=f"{cfg.api_prefix}/portfolio", tags=["Portfolio"])

    # Orders endpoints
    from interface.api.routers.orders import router as orders_router

    app.include_router(orders_router, prefix=f"{cfg.api_prefix}/orders", tags=["Orders"])

    # Risk endpoints
    from interface.api.routers.risk import router as risk_router

    app.include_router(risk_router, prefix=f"{cfg.api_prefix}/risk", tags=["Risk"])

    # Audit trail endpoints
    from interface.api.routers.audit import router as audit_router
    ...
    from interface.api.ws.replay import router as ws_replay_router
