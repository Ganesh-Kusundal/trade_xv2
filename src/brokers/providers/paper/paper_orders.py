...
class PaperOrders:
    ...
    def _place_via_oms(
        ...
        # Nested re-entry: OMS submit_fn → PaperGateway → here with the same
        # OrderManager + correlation_id. Re-calling place_order hits idempotency
        # and returns the outer stub (OPEN, unfilled). Fill only.
        get_by_corr = getattr(self._order_manager, "get_order_by_correlation", None)
        existing = (
            get_by_corr(cmd.correlation_id)
            if callable(get_by_corr) and cmd.correlation_id
            else None
        )
        if existing is not None:
            ...
            filled = _fill(cmd)
            # Keep OMS book order_id so record_trade does not buffer against a
            # not-yet-upserted PPR id while the outer place still holds the stub.
            if getattr(existing, "order_id", None):
                filled = replace(filled, order_id=existing.order_id)
            with self._lock:
                self._orders.append(filled)
            if is_market and filled.status in (
                OrderStatus.FILLED,
                OrderStatus.PARTIALLY_FILLED,
            ):
                self._trade_seq += 1
                trade = Trade(
                    ...
                    price=fill_price,
                    trade_value=fill_price * Decimal(int(filled.filled_quantity)),
                    timestamp=get_current_clock().now(),
                    product_type=product_type,
                )
                with self._lock:
