...
class DhanConnectionSettings(BrokerSettings):
    ...
    def resolved_token_state_dir(self) -> Path:
        ...
        # src/brokers/providers/dhan/config/settings.py → … → project root (Trade_XV2/)
...
class DhanSettingsLoader(SettingsLoaderBase):
    ...
    Mirrors the :class:`brokers.providers.upstox.auth.config.UpstoxSettingsLoader` pattern
    ...
    def _load_resilience_config(
        ...
        """
        from brokers.providers.dhan.config import ENV_PREFIX, DhanConfigLoader, load_from_env_file
