"""Dhan broker domain models — Dhan-only types with canonical domain imports.

Dhan-specific types (``DhanInstrument``, ``MarginRequest``, etc.) are defined
here. Canonical domain types (``Order``, ``Position``, ``Trade``, etc.) must
be imported from ``domain.entities`` directly — not from this module.

Exchange, InstrumentType, OptionType are re-exported from domain.market_enums
for convenience within the Dhan adapter package.
"""
