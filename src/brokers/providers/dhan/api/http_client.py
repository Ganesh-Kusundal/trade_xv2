...
from brokers.providers.dhan.config import DEFAULT_CONFIG, DhanResilienceConfig
from brokers.providers.dhan.config.http_client_config import DhanHttpClientConfig
from brokers.providers.dhan.exceptions import AuthenticationError, DhanError, RateLimitError
from brokers.providers.dhan.resilience.metrics import (
    dhan_errors_total,
...
class _DhanRateLimitAdapter:
    """Adapt MultiBucketRateLimiter to ResilientHttpTransport (DP-01)."""

    def __init__(self, client: "DhanHttpClient") -> None:
        self._client = client

    def acquire(self, bucket: str, tokens: int = 1, timeout: float = 5.0) -> bool:
        limiter = self._client._rate_limiter
        if limiter is None:
            return True
        return bool(limiter.acquire(bucket, tokens=tokens, timeout=timeout))
...
class DhanHttpClient:
    ...
    def __init__(
        ...
        # New: structured config object (takes precedence over individual params)
        http_config: DhanHttpClientConfig | None = None,
    ) -> None:
        # Apply structured config if provided (overrides individual params)
        if http_config is not None:
            client_id = http_config.client_id or client_id
            access_token = http_config.access_token or access_token
            base_url = http_config.base_url or base_url
            timeout = http_config.timeout
            token_refresh_fn = http_config.token_refresh_fn or token_refresh_fn  # type: ignore[assignment]
            enable_retry = http_config.enable_retry
            circuit_breaker = http_config.circuit_breaker or circuit_breaker
            read_circuit_breaker = http_config.read_circuit_breaker or read_circuit_breaker
            write_circuit_breaker = http_config.write_circuit_breaker or write_circuit_breaker
            admin_circuit_breaker = http_config.admin_circuit_breaker or admin_circuit_breaker
            session = http_config.session or session
            _rate_limiter = http_config.rate_limiter or _rate_limiter
            _circuit_breakers = http_config.circuit_breakers or _circuit_breakers
            config = http_config.config or config
        ...
        # ponytail: category-specific CBs stay on DhanHttpClient; transport shell
        # unifies rate-limit acquire + retry hooks (DP-01).
        self._resilient_transport = self._build_resilient_transport()

    def _build_resilient_transport(self):
        from brokers.common.http.resilient_transport import ResilientHttpTransport

        return ResilientHttpTransport(
            rate_limiter=_DhanRateLimitAdapter(self),
        ...
        try:
            ...
            if new_token:
                ...
                publish_token_lifecycle_event(
                    None, "TOKEN_REFRESHED", broker_id="dhan", client_id=self.client_id
                )
                return True
            else:
                ...
                publish_token_lifecycle_event(
                    None, "TOKEN_EXPIRED", broker_id="dhan", reason="refresh_returned_none"
                )
                return False
            ...
            if "once every 2 minutes" in error_msg or "rate limit" in error_msg.lower():
                ...
                publish_token_lifecycle_event(
                    None, "TOKEN_EXPIRED", broker_id="dhan", reason=error_msg
                )
        return False
    ...
    def _request(self, method: str, endpoint: str, json: dict | None = None) -> dict[str, Any]:
        ...
        # Rate limit token acquisition via shared transport shell (DP-01)
        ...
        if _is_ambiguous_write(method, endpoint):
            # ponytail: allow exactly one 401 refresh retry on writes; transport
            # auto-retry stays disabled to avoid duplicate posts.
            max_attempts = 2 if self._token_refresh_fn else 1
        else:
