...
----------------------
The :class:`brokers.providers.dhan.identity.DhanInstrumentRef` dataclass already
...
:class:`brokers.providers.dhan.exceptions.DhanIdentityError`. The error message
...
from brokers.providers.dhan.identity import DHAN_SEGMENTS, is_dhan_segment
...
def assert_dhan_segment(segment: str, *, context: str = "") -> None:
    ...
    present in :data:`brokers.providers.dhan.identity.DHAN_SEGMENTS`. Upstox
...
def assert_dhan_identity(
    ...
    *,
    ...
    2. ``assert_dhan_identity(ref)`` — pass a
       :class:`brokers.providers.dhan.identity.DhanInstrumentRef` (or any object
       that exposes ``security_id`` and ``exchange_segment``) directly;
       the helper will unpack it.

    This is the same check that
    :class:`brokers.providers.dhan.identity.DhanInstrumentRef.__post_init__`
