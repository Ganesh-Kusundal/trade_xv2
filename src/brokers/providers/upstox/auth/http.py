...
class UpstoxHttpClient:
    ...
    def __init__(
        ...
        *,
        ...
        # not in the HTTP client itself. See brokers.providers.upstox.auth.context.
    ...
    def _execute_request(
        ...
        *,
        ...
        total_attempts = self._resolve_total_attempts(method, url)
        last_auth_error: UpstoxAuthError | None = None
        last_exc: UpstoxApiError | None = None
        auth_refreshed = False

        for attempt in range(total_attempts):
            self._acquire_rate_limit(bucket, url)
            ...
            try:
                ...
                last_exc, should_raise = self._handle_transport_failure(
                    exc, method, url, attempt, total_attempts
                )
                if should_raise:
            ...
            if resp.status_code >= 400:
                handled, last_auth_error, last_exc, auth_refreshed = self._handle_http_error(
                    resp, method, url, attempt, total_attempts, auth_refreshed
                )
                if handled:
                    continue  # retriable — auth refresh or transient error
                # Permanent failure — raise immediately
                if last_auth_error is not None:
                    raise last_auth_error
                if last_exc is not None:
                    raise last_exc

            body = self._parse_success_body(resp, method, url)
            if body is not None:
                return body
    ...
    def _resolve_total_attempts(self, method: str, url: str) -> int:
        """Determine retry count: ambiguous writes get 1 attempt, others get max_retries + 2."""
        if _is_ambiguous_write(method, url):
            return 1
        return self._max_retries + 2

    def _acquire_rate_limit(self, bucket: str, url: str) -> None:
        """Acquire rate-limit token; raise on timeout."""
        from brokers.common.http.resilient_transport import EndpointPolicy

        policy = EndpointPolicy(bucket=bucket)
        try:
    ...
    def _handle_transport_failure(
        ...
        attempt: int,
        total_attempts: int,
    ) -> tuple[UpstoxApiError, bool]:
        """Handle network/transport errors. Returns (error, should_raise)."""
        ...
        if attempt < total_attempts - 1:
            ...
            return last_exc, False  # retry
        return last_exc, True  # raise
    ...
    def _handle_http_error(
        ...
        total_attempts: int,
        auth_refreshed: bool,
    ) -> tuple[bool, UpstoxAuthError | None, UpstoxApiError | None, bool]:
        """Handle 4xx/5xx HTTP responses. Returns (handled, auth_err, api_err, auth_refreshed).

        ``handled=True`` means the caller should retry (transient error or auth refresh).
        ``handled=False`` means the caller should raise the returned error immediately.
        """
        # Auth failures (401/403) — retry once after token refresh
        if resp.status_code in (401, 403):
            auth_result = self._handle_auth_failure(
                resp, method, url, auth_refreshed
            )
            if auth_result is not None:
                return True, None, None, auth_result
            # No refresh available — permanent failure, do NOT retry
            auth_err = UpstoxAuthError(
                f"Upstox API {method} {url} failed: HTTP {resp.status_code}",
                resp.status_code,
                resp.text,
            )
            return False, auth_err, None, auth_refreshed
        ...
        if is_transient and attempt < total_attempts - 1:
            ...
            return True, None, api_err, auth_refreshed

        # Permanent failure (non-transient 4xx or no retries left) — do NOT retry
        api_err = UpstoxApiError(
            f"Upstox API {method} {url} failed: HTTP {resp.status_code}",
            resp.status_code,
            resp.text,
        )
        return False, None, api_err, auth_refreshed
    ...
    def _handle_auth_failure(
        ...
        """Handle 401/403: attempt token refresh once. Returns new auth_refreshed or None."""
        if not auth_refreshed and self._on_auth_failure is not None and self._on_auth_failure():
            logger.info(
    ...
    def _parse_success_body(self, resp: Any, method: str, url: str) -> dict[str, Any] | None:
        """Parse 2xx response body; return None if body signals API-level failure."""
        body = resp.json() if resp.text else {}
        if isinstance(body, dict) and str(body.get("status", "")).lower() in {"failure", "error"}:
