...
def _resolve_ohlc(data: dict[str, Any]) -> dict[str, Any]:
    """Extract OHLC data — prefer full ohlc dict, fall back to live_ohlc, then individual fields."""
    ...
    return {**ohlc, "close": close, "volume": data.get("volume") or ohlc.get("volume")}


def _resolve_symbol(data: dict[str, Any], map_key: str = "") -> str:
    """Resolve the canonical symbol from instrument data or map_key fallback."""
...
def _resolve_bid_ask(data: dict[str, Any]) -> tuple[Any, Any]:
...
def _quote_from_instrument_dict(
    ...
    *,
    ...
    ohlc = _resolve_ohlc(data)
    symbol = _resolve_symbol(data, map_key)
    bid, ask = _resolve_bid_ask(data)
