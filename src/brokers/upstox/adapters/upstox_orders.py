"""UpstoxOrders — place, cancel, modify, and query orders.
...
class UpstoxOrders:
    ...
    Example::
        ...
        gw = UpstoxOrders(broker, order_command)
        response = gw.place_order("RELIANCE", "NSE", "BUY", 1)
        book = gw.get_orderbook()
    """

    def __init__(self, broker: Any, order_command: Any) -> None:
        """Initialize with broker facade and order command adapter."""
        self._broker = broker
    ...
    def get_order(self, order_id: str) -> Order | None:
        ...
        if order_query is not None:
            order = order_query.get_order(order_id)
    ...
    def get_orderbook(self) -> list[Order]:
        ...
        if order_query is None:
            return []
        return order_query.get_order_list()
