# Current-State Architecture — Trade_XV2
...
Trade_XV2 is a **real-money trading operating system**. It must:
...
## 6. Runtime Simulation Notes (one live cycle)

Assumptions: CLI `build_runtime("dhan")`, env leaves `ORCHESTRATOR_DRY_RUN` default `"1"`, both brokers not present.

1. Compose builds `BrokerService`; gateway **not** yet authenticated.
2. Factory touches `bs.active_broker` → lazy connect/probe.
3. Parity gate **skipped** (hardcoded default) — silent.
4. Orchestrator wires with `dry_run=True` — autonomous path simulates, does not place live.
5. Operator places via OMS spine → risk + idempotency (memory) → submit_fn → broker.
6. Fill via WS → `on_trade` → recorder → position. If WS drop: recon timer runs → **reports** drift only → local book stays wrong.
7. Process crash mid-submit → restart → same `correlation_id` **not** remembered → second broker order possible.

Unsafe assumptions surfaced: “parity ran”, “recon heals”, “daily PnL is daily realized”, “idempotency is durable”, “dry_run off means intentional live”.

---

## 7. What Works

- Domain layer purity and port placement.
- Brokers ports-and-adapters + enforced connect funnel + architecture test against private factory bypass.
- Single live order spine through `OrderManager.place_order` for most paths.
- Shared `StrategyPipeline` / `FeaturePipeline` / cost helpers as the analytics core.
- EventBus as domain port with one production impl; replay_mode disables double-dispatch.
- Consolidated rate limiter in `infrastructure/resilience/rate_limiter.py`.
- Schema package split under `interface/api/schemas/` is consistent (no leftover monolithic `schemas.py` imports).
