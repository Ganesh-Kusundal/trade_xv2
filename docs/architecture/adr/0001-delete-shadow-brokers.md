...
## Context
`brokers/dhan/gateway.py` and `brokers/dhan/orders.py` at the repo root are divergent
duplicates of `src/brokers/dhan/gateway.py` / `src/brokers/dhan/orders.py`. The root
copy imports `from tradex.runtime.capabilities import ...` and `from brokers.dhan.connection`,
...
## Decision
1. Delete `brokers/dhan/gateway.py` and `brokers/dhan/orders.py`.
2. Keep `_bootstrap.py` for now (it protects against other shadowed editable installs)
   but add a test that `import brokers.dhan.gateway` resolves to `src/brokers/dhan/gateway.py`.
3. Grep the whole repo for any import of the root `brokers.dhan.gateway` / `brokers.dhan.orders`;
...
- Test: `tests/architecture/test_no_shadow_brokers.py` asserts the resolved `__file__`
  of `brokers.dhan.gateway` starts with `src/`.
- CI: import-linter contract forbids `brokers.dhan.gateway` resolving outside `src/`.
