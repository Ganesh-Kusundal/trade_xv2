"""Dhan depth merge, subscription loop, and rate-limit invariants (offline).

Guarantees:
* Partial WS depth merges REST asks/bids when one side is missing.
* Subscriptions use the feed WebSocket loop, not the caller thread.
* Rate-limit tokens map read→market_data and write→orders.
* SDK depth list normalizes to {bids, asks}.
* DEPTH_20 bridge uses depth.symbol when event symbol is None.
* Quote endpoint minimum interval is 1.0 s.
"""
...
class TestCompleteDepthSnapshot:
    ...
    def test_none_ws_depth_returns_rest(self):
        from brokers.providers.dhan.market_data.market_data import merge_depth_with_rest
