...
def test_bootstrap_reuses_valid_json_token_without_totp(env_file, tmp_path):
    ...
    with patch("brokers.providers.dhan.identity.factory._generate_totp_token", fake_generate):
        factory = BrokerFactory()
        auth, token = factory._create_auth(
            __import__(
                "brokers.providers.dhan.config.settings", fromlist=["DhanSettingsLoader"]
...
def test_generate_totp_delegates_to_client():
    ...
    from brokers.providers.dhan.identity.factory import _generate_totp_token

    with patch("brokers.providers.dhan.auth.totp_client.DhanTotpClient") as mock_cls:
