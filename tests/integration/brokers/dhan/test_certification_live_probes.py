"""Live certification probe integration tests for Dhan (marker-gated)."""
...
@pytest.mark.dhan
def test_dhan_live_token_probes() -> None:
    session = _open_or_skip("dhan")
    try:
...
@pytest.mark.live_readonly
@pytest.mark.dhan
def test_dhan_live_reconnect_probes() -> None:
    session = _open_or_skip("dhan")
    try:
