...
class TestOrderToPositionFlow:
    ...
    def test_full_close_flattens_position(self, trading_context, event_capturer):
        ...
        sell_result = trading_context.order_manager.place_order(
            cmd_sell, submit_fn=_make_submit_fn(Decimal("105.0"))
        )
        sell_trade = Trade(
            trade_id="SELL-002",
            order_id=sell_result.order.order_id,
...
class TestKillSwitchWithActivePositions:
    ...
    def test_kill_switch_blocks_new_orders_with_positions(self, trading_context):
        ...
        cmd_new = OmsOrderCommand(
            ...
            quantity=5,
            price=Decimal("200.0"),
            correlation_id="kill-active-new",
        )
    ...
    def test_kill_switch_allows_position_closure(self, trading_context):
        ...
        trading_context.order_manager.place_order(
            cmd_sell, submit_fn=_make_submit_fn(Decimal("105.0"))
        )
    ...
    def test_kill_switch_deactivation_resumes_trading(self, trading_context):
        ...
        result_blocked = trading_context.order_manager.place_order(
            cmd_blocked, submit_fn=_make_submit_fn()
        )
        ...
        result_allowed = trading_context.order_manager.place_order(
            cmd_allowed, submit_fn=_make_submit_fn()
        )
...
class TestMultiSymbolPortfolioTrading:
    ...
    def test_portfolio_pnl_aggregation(self, trading_context):
        ...
        total_pnl = sum(p.unrealized_pnl for p in trading_context.position_manager.get_positions())
    ...
    def test_mixed_long_short_portfolio(self, trading_context):
        ...
        short_result = trading_context.order_manager.place_order(
            cmd_short, submit_fn=_make_submit_fn(Decimal("200.0"))
        )
        trading_context.order_manager.record_trade(
            _make_trade(short_result.order, Decimal("200.0"))
        )
...
class TestRiskMidFlowEnforcement:
    ...
    def test_daily_loss_limit_halts_trading(self, tmp_path):
        ...
        for i in range(10):
            ...
            sell_result = ctx.order_manager.place_order(
                cmd_sell, submit_fn=_make_submit_fn(Decimal("95.0"))
            )
            sell_trade = Trade(
