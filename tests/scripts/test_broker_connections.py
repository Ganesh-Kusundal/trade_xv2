...
def test_dhan_connection() -> dict:
    ...
    try:
        ...
        try:
            ...
            latency = (time.time() - t0) * 1000
            console.print(f"[green]✓ Balance: ₹{balance.available_balance:.2f} ({latency:.0f}ms)[/green]")
            results['portfolio'] = {
                'status': 'PASS',
        ...
        try:
            ...
            latency = (time.time() - t0) * 1000
            console.print(f"[green]✓ RELIANCE LTP: ₹{quote.ltp}, Volume: {quote.volume:,} ({latency:.0f}ms)[/green]")
            results['quote'] = {
                'status': 'PASS',
                'ltp': quote.ltp,
                'volume': quote.volume,
        ...
        try:
            if hasattr(gateway, 'options'):
                ...
                latency = (time.time() - t0) * 1000
                console.print(f"[green]✓ NIFTY Options: {len(expiries)} expiries ({latency:.0f}ms)[/green]")
                results['options'] = {
                    'status': 'PASS',
                ...
                console.print("[yellow]⚠ Options via extended capabilities (testing future_chain)...[/yellow]")
                try:
                    t0 = time.time()
                    contracts = gateway.future_chain('NIFTY', 'NSE')
                    latency = (time.time() - t0) * 1000
                    console.print(f"[green]✓ NIFTY Futures: {len(contracts)} contracts ({latency:.0f}ms)[/green]")
                    results['options'] = {
                        'status': 'PASS',
                    ...
                    results['options'] = {'status': 'SKIP', 'reason': 'Not available via standard interface'}
            else:
    ...
    try:
        ...
        try:
            if hasattr(gateway, 'portfolio'):
                ...
                console.print("[yellow]⚠ Portfolio API not available on Upstox gateway (analytics-only mode)[/yellow]")
                results['portfolio'] = {'status': 'SKIP', 'reason': 'Not available in analytics-only mode'}
        except Exception as e:
            console.print(f"[red]✗ Portfolio test failed: {e}[/red]")
            results['portfolio'] = {'status': 'FAIL', 'error': str(e)}
...
def print_summary(dhan_results: dict, upstox_results: dict) -> None:
    ...
    for test_name, result in dhan_results.items():
        if isinstance(result, str):
            ...
            details = ', '.join(f"{k}={v}" for k, v in result.items() if k not in ('status', 'error'))
            if 'error' in result:
    ...
    console.print(table)
    dhan_pass = sum(1 for r in dhan_results.values() if (isinstance(r, dict) and r.get('status') == 'PASS'))
    ...
    console.print(table)
    upstox_pass = sum(1 for r in upstox_results.values() if (isinstance(r, dict) and r.get('status') == 'PASS'))
    ...
    if all_pass:
        ...
        console.print("[bold yellow]⚠ SOME TESTS FAILED OR SKIPPED - CHECK DETAILS ABOVE[/bold yellow]")
    console.print("=" * 80)
