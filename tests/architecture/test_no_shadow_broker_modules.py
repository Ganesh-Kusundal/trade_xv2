...
The repo previously carried divergent duplicates of ``src/brokers/dhan/*`` at the
repo root (``brokers/dhan/gateway.py``, ``brokers/dhan/orders.py``). They were kept
...
- an actual ``src/`` module (``brokers.dhan.wire``) must resolve under ``src/``;
- the formerly-shadowed names (``gateway``, ``orders``) must not exist under
  ``brokers.dhan`` at all;
- no ``*.py`` may exist under a repo-root ``brokers/dhan/`` directory.
"""
...
import brokers.dhan.wire as wire_module
...
def test_dhan_wire_resolves_to_src() -> None:
    ...
    assert "src/brokers/dhan/wire.py" in str(resolved), resolved


@pytest.mark.architecture
def test_no_shadow_dhan_module_names() -> None:
    """The orphaned shadow module names must not exist under brokers.dhan."""
    import brokers.dhan as dhan_pkg
...
def test_no_root_shadow_broker_modules() -> None:
    """No *.py shadow modules may exist under a repo-root brokers/dhan/ directory."""
